...
# ================================================================
# evaluate_predictions_report.R  (FULL - fixed/tuned + ROC/AUC + PR)
# ================================================================
...
EvaluatePredictionsReport <- function(
  ...
  ## ============================================================
  ## SECTION: Local helpers (CRAN-safe defaults)                  #$$$$$$$$$$$$$
  ## ============================================================
  `%||%` <- function(a, b) if (is.null(a) || !length(a)) b else a  #$$$$$$$$$$$$$
  
  ## ============================================================
  ## SECTION: Report plot toggles via SONN R6 (CRAN-safe)          #$$$$$$$$$$$$$
  ## ============================================================
  .allow_plot <- function(name) {                                  #$$$$$$$$$$$$$
    f <- tryCatch(SONN$viewEvaluatePredictionsReportPlots, error = function(e) NULL)  #$$$$$$$$$$$$$
    if (is.function(f)) {                                          #$$$$$$$$$$$$$
      return(isTRUE(tryCatch(f(name), error = function(e) FALSE)))  #$$$$$$$$$$$$$
    }                                                              #$$$$$$$$$$$$$
    FALSE                                                          #$$$$$$$$$$$$$
  }                                                                #$$$$$$$$$$$$$
  ...
  # FIX: plots should live in <output_root>/R/reports (Techila_MVP_Test/R/reports)
  #      If output_root is NULL/empty, keep existing plot-dir behavior.
  plot_dir <- if (!is.null(output_root) && nzchar(output_root)) {
    file.path(normalizePath(output_root, winslash = "/", mustWork = FALSE), "R", "reports")
  } else {
    file.path(ddesonn_plots_dir(output_root), "reports", "EvaluatePredictionsReportPlots")
  }
  
  dir.create(reports_dir, recursive = TRUE, showWarnings = FALSE)   # always create (idempotent)
  dir.create(plot_dir, recursive = TRUE, showWarnings = FALSE)      # always create (idempotent)
  ...
  # Helper: find an existing file across candidates (no writing)
  ...
  if (isTRUE(.allow_plot("pred_vs_error_scatter")) && max_points > 0) {  #$$$$$$$$$$$$$
    tryCatch({
  ...
  if (identical(mode, "binary")) {
    ...
    if (isTRUE(.allow_plot("roc_curve")) && !is.null(roc_df) && nrow(roc_df) > 1) {  #$$$$$$$$$$$$$
      try({
    ...
    artifacts <- list()  #$$$$$$$$$$$$$
    if (isTRUE(.allow_plot("binary_fixed_plots"))) {  #$$$$$$$$$$$$$
      artifacts$fixed <- maybe_plot_binary("accuracy", y_pred_fixed, 0.5, "_fixed")  #$$$$$$$$$$$$$
    }  #$$$$$$$$$$$$$
    if (isTRUE(.allow_plot("binary_tuned_plots"))) {  #$$$$$$$$$$$$$
    ...
    # Save PR curve plot (toggle)
    pr_png <- file.path(plot_dir, "pr_curve.png")
    if (isTRUE(.allow_plot("pr_curve")) && !is.null(pr_obj)) {  #$$$$$$$$$$$$$
      try({
    ...
    # Confusion heatmap (legacy) - toggle
    ...
    if (isTRUE(.allow_plot("legacy_conf_heatmap"))) {  #$$$$$$$$$$$$$
      try({
  ...
  if (isTRUE(.allow_plot("multiclass_heatmap"))) {  #$$$$$$$$$$$$$
    tryCatch({
