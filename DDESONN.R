  ...
  public = list(
    ...
    predict = function(Rdata, weights, biases, activation_functions_predict, verbose=FALSE, debug=FALSE) {
      ...
      activation_functions_predict <- learn_predict_normalize_activations(
        ...
        max(1L, length(weights))   # or max(1L, self$num_layers) if you prefer
      )
    ...
    train_network = function(Rdata, labels,  X_train = NULL, y_train = NULL, lr, num_networks, CLASSIFICATION_MODE, num_epochs, model_iter_num, update_weights, update_biases, ensemble_number, do_ensemble, reg_type, activation_functions, dropout_rates, optimizer, beta1, beta2, epsilon, lookahead_step, loss_type, sample_weights, X_validation, y_validation, validation_metrics, threshold_function, ML_NN, train, verbose) {
      ...
      # ------------------------------------------------------------------
      # Some downstream diagnostics (e.g. training-metric fallbacks when no
      # validation data is supplied) still refer to `X_train`/`y_train`.
      # Earlier revisions expected those symbols to exist in the caller's
      # environment, which isn't true when the high-level API only passes
      # `Rdata`/`labels`.  Create explicit aliases so every branch inside the
      # method has reliable access to the training set.
...
DDESONN <- R6Class(
  ...
  public = list(
    ...
    Rdata, labels, num_networks, update_weights, update_biases, preprocessScaledData,
    X_validation, y_validation, validation_metrics, lr, CLASSIFICATION_MODE,
    ensemble_number, model_iter_num, num_epochs, threshold, threshold_function,
    learn_results, predicted_output_list, all_best_val_probs, all_best_val_labels,
    all_best_val_prediction_time, learn_time, prediction_time_list, run_id,
    all_predicted_outputAndTime, all_weights, all_biases, all_activation_functions,
    all_activation_functions_predict, all_best_train_acc, all_best_epoch_train,
    all_best_val_acc, all_best_val_epoch, best_weights_on_lastest_weights_off,
    ML_NN, train, grouped_metrics, viewTables, verbose
      ...
      for (i in 1:length(self$ensemble)) {
        ...
        if (train) {
          ...
          # local, block-scoped coercers (no new external helpers)
          as_prob_matrix <- function(x) {
            if (is.null(x)) return(NULL)
            if (is.list(x) && !is.null(x$predicted_output)) x <- x$predicted_output
            if (is.list(x) && !is.null(x$probs))            x <- x$probs
            if (is.data.frame(x))                           x <- as.matrix(x)
            if (is.vector(x) && is.atomic(x))               x <- matrix(as.numeric(x), ncol = 1L)
            if (is.matrix(x))                               return(x)
            if (is.atomic(x))                               return(matrix(as.numeric(x), ncol = 1L))
            NULL
          }
          to_label_vec <- function(y) {
            if (is.null(y)) return(NULL)
            if (is.matrix(y))     return(y[, 1])
            if (is.data.frame(y)) return(y[[1]])
            if (is.atomic(y))     return(y)
            NULL
          }
          align_len <- function(a, b) {
            if (is.null(a) || is.null(b)) return(list(a = a, b = b))
            n <- min(length(a), length(b))
            list(a = a[seq_len(n)], b = b[seq_len(n)])
          }
          
          # NOTE: tuning runs only in binary classification + when validation is provided
          if (!is.null(X_validation) && !is.null(y_validation) &&
              isTRUE(validation_metrics) && identical(CLASSIFICATION_MODE, "binary")) {
            
            # choose snapshot to tune: prefer best_val_*; else last-epoch predictions
            if (!is.null(best_val_probs) && !is.null(best_val_labels)) {
              probs_for_tuning  <- as_prob_matrix(best_val_probs)
              labels_for_tuning <- as.integer(to_label_vec(best_val_labels))
            } else {
              # fallback to latest predictions
              probs_for_tuning  <- as_prob_matrix(single_predicted_output)
              labels_for_tuning <- as.integer(to_label_vec(y_validation))
              
              # length alignment
              if (!is.null(probs_for_tuning)) {
                al <- align_len(labels_for_tuning, as.vector(probs_for_tuning))
                labels_for_tuning <- al$a
                probs_for_tuning  <- if (!is.null(al$b)) matrix(al$b, ncol = 1L) else NULL
              }
            }
            
            # If we still don't have valid probs/labels, skip tuning gracefully
            if (!is.null(probs_for_tuning) && !is.null(labels_for_tuning) &&
                length(labels_for_tuning) > 0L && nrow(probs_for_tuning) > 0L) {
              ...
              # Guard against primitive-attribute warning
              # (no-op here; just store on the model as scalar)
              self$ensemble[[i]]$chosen_threshold <- chosen_threshold
            } else {
              if (isTRUE(verbose)) message("[tuning] Skipping: missing probs/labels after coercion.")
            }
          }
          ...
          safe_ncol <- function(x) {
            ...
            if (is.atomic(x))     return(1L)  # vectors/scalars count as 1 col (binary probs)
            0L
          }
          ...
          # --- Which probs/labels do we score with? (snapshot vs last-epoch) ---
          if (best_weights_on_lastest_weights_off && !is.null(best_val_probs) && !is.null(best_val_labels)) {
            probs           <- as_prob_matrix(best_val_probs)
            targets         <- matrix(as.integer(to_label_vec(best_val_labels)), ncol = 1L)
            prediction_time <- best_val_prediction_time
            cat("[calculate_performance] Using best validation snapshot (@ best epoch)\n")
          } else {
            probs           <- as_prob_matrix(single_predicted_output)
            targets         <- if (!is.null(labels)) matrix(as.integer(to_label_vec(labels)), ncol = 1L) else NULL
            prediction_time <- single_prediction_time
            cat("[calculate_performance] Using last-epoch predictions\n")
          }
          
          # If still NULL (e.g., skipped path), bail out gracefully for this model
          if (is.null(probs) || is.null(targets)) {
            if (isTRUE(verbose)) message("[calculate_performance] Skipping metrics for this model due to NULL probs/labels.")
            next
          }
        ...
        self$store_metadata(
          single_predicted_outputAndTime,
          actual_values = NULL, do_ensemble = NULL,
          self$input_size, self$output_size, self$N,
          ...
          X = NULL, y = NULL, X_test_scaled = NULL, y_test = NULL,
          all_weights, all_biases, artifact_names, artifact_paths,
          validation_metrics = validation_metrics,
          single_activation_functions, single_activation_functions_predict,
          self$dropout_rates, self$hidden_sizes, self$ML_NN,
          best_val_prediction_time, best_train_acc, best_epoch_train, best_val_acc, best_val_epoch,
          performance_metric, relevance_metric, NULL
        )
        
      } # end for i
      
      # =========================
      # Extract metrics containers
      # =========================
      ...
      if (grouped_metrics) {
        ...
        if (ensemble_number >= 1 && length(self$ensemble) > 1) {
          ...
          if (verbose || viewTables) {
            if (!is.null(perf_df)) {
              emit_table(utils::head(perf_df, 12), title = "--- performance_long_df (head) ---", verbose = verbose, viewTables = viewTables)
            }
            if (!is.null(relev_df)) {
              emit_table(utils::head(relev_df, 12), title = "--- relevance_long_df (head) ---", verbose = verbose, viewTables = viewTables)
            }
          }
          ...
          if (verbose) {
            if (!is.null(perf_group_summary)) {
              emit_table(perf_group_summary, title = "=== PERFORMANCE group summary ===", verbose = verbose, viewTables = viewTables)
            }
            if (!is.null(relev_group_summary)) {
              emit_table(relev_group_summary, title = "=== RELEVANCE group summary ===", verbose = verbose, viewTables = viewTables)
            }
            if (!is.null(group_perf)) {
              emit_table(group_perf$metrics, title = "=== GROUPED PERFORMANCE metrics ===", verbose = verbose, viewTables = viewTables)
            }
            if (!is.null(group_relev)) {
              emit_table(group_relev$metrics, title = "=== GROUPED RELEVANCE metrics ===", verbose = verbose, viewTables = viewTables)
            }
          }
        }
      } # end grouped_metrics
      ...
      # Return the lists of plots and evaluation details
      return(list(
