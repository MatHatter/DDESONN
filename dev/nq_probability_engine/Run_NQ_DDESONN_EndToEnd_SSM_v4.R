...
# ============================================================================
# NQ INTRADAY PROBABILITY ENGINE — END-TO-END DDESONN + CAUSAL SSM v4
# ============================================================================
...
#   * train a 16-state causal SSM end-to-end on the previous 48 five-minute bars
#
# RStudio use:
...
results_path <- if (length(arguments) >= 3L) arguments[[3L]] else {
  "NQ_DDESONN_EndToEnd_SSM_Results"
}
...
# Compatibility helpers retained for older encoder APIs. The v4 training path
# below uses the package's integrated sequence_encoder = "ssm" interface.
...
message("RUN SCRIPT VERSION: END-TO-END SSM v4")
...
# Map the causal sequence for each ObservationId to every associated target,
# stop, direction and horizon row. DDESONN receives these arrays directly so
# its SSM parameters can be updated by supervised backpropagation.
sequence_row <- match(dataset$ObservationId, observation_table$ObservationId)
if (anyNA(sequence_row)) {
  stop("A causal sequence could not be mapped to a modeling row.", call. = FALSE)
}

message("Building row-aligned Train, Validation and Holdout sequence arrays...")
sequence_train <- observation_sequences[
  sequence_row[train_rows], , , drop = FALSE
]
sequence_validation <- observation_sequences[
  sequence_row[validation_rows], , , drop = FALSE
]
sequence_holdout <- observation_sequences[
  sequence_row[holdout_rows], , , drop = FALSE
]

if (any(!is.finite(sequence_train)) ||
    any(!is.finite(sequence_validation)) ||
    any(!is.finite(sequence_holdout))) {
  stop("A row-aligned SSM sequence array contains non-finite values.", call. = FALSE)
}

# Release the dense bar-level intermediates before model training. The three
# aligned arrays above are the only temporal objects required by DDESONN.
rm(observation_sequences, sequence_feature_matrix)
invisible(gc())

model_matrix <- feature_matrix
...
audit <- data.frame(
  ...
  SequenceFeatures = dim(sequence_train)[3L],
  SSMStateDimension = 16L,
  SequenceBars = sequence_length,
  stringsAsFactors = FALSE
)
utils::write.csv(
  audit,
  file.path(results_path, "NQ_DDESONN_EndToEnd_SSM_split_audit.csv"),
  row.names = FALSE
)
print(audit)
...
run_arguments <- list(
  ...
  validation = list(
    x = x_validation,
    y = y_validation,
    sequence_data = sequence_validation
  ),
  test = list(
    x = x_holdout,
    y = y_holdout,
    sequence_data = sequence_holdout
  ),
  sequence_encoder = "ssm",
  sequence_data = sequence_train,
...
message(sprintf(
  paste(
    "Training input: %d rows x %d static features;",
    "sequences=%d x %d x %d; range=[%.4f, %.4f]"
  ),
  nrow(x_train), ncol(x_train), dim(sequence_train)[1L],
  dim(sequence_train)[2L], dim(sequence_train)[3L],
  min(x_train), max(x_train)
))
message("Training DDESONN and causal SSM end-to-end...")
fit <- do.call(DDESONN::ddesonn_run, run_arguments)
if (is.null(fit$model)) {
  stop("DDESONN completed without returning a final model.", call. = FALSE)
}

prediction_arguments <- list(
  fit$model,
  x_holdout,
  aggregate = "mean",
  type = "response"
)
predict_parameter_names <- names(formals(DDESONN::ddesonn_predict))
if ("sequence_data" %in% predict_parameter_names) {
  prediction_arguments$sequence_data <- sequence_holdout
}
prediction_result <- do.call(
  DDESONN::ddesonn_predict,
...
holdout_metrics <- data.frame(
  Model = "DDESONN + end-to-end causal SSM",
...
utils::write.csv(
  holdout_metrics,
  file.path(results_path, "NQ_DDESONN_EndToEnd_SSM_holdout_metrics.csv"),
  row.names = FALSE
)
utils::write.csv(
  per_horizon_metrics,
  file.path(results_path, "NQ_DDESONN_EndToEnd_SSM_holdout_metrics_by_horizon.csv"),
  row.names = FALSE
)
utils::write.csv(
  calibration,
  file.path(results_path, "NQ_DDESONN_EndToEnd_SSM_holdout_calibration.csv"),
  row.names = FALSE
)
utils::write.csv(
  holdout_predictions,
  file.path(results_path, "NQ_DDESONN_EndToEnd_SSM_holdout_predictions.csv"),
  row.names = FALSE
)
...
saveRDS(
  list(
    ...
    sequence_feature_columns = c(
      "Return5m", "Return15m", "Return30m", "Return60m",
      "RangePct", "BodyPct", "UpperWickPct", "LowerWickPct",
      "AtrPct", "CloseLocation", "LogVolume", "TimeSin", "TimeCos"
    ),
  ...
  file.path(results_path, "NQ_DDESONN_EndToEnd_SSM_model.rds")
)

message("End-to-end DDESONN + SSM holdout run completed.")
